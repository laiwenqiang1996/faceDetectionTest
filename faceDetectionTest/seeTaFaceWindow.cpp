...
void faceDetectionFunc(cv::Mat &mat, std::vector<SeetaPointF>& facePoints)
{
	...
	static seeta::ModelSetting::Device device = seeta::ModelSetting::AUTO;
	//初始化模型，需要注意的是，模型文件同一时刻只能打开一次，并且加载模型时需要一定的时间，所以需要进行加锁处理
	myMutex.lock();
	static seeta::ModelSetting FD_model("./model/fd_2_00.dat", device, 0);
	//关于81点定位之后，得到的人脸相似度结果不理想的原因猜想：
	//图片的尺寸太小，导致81点都挤在了一起，然后进行人脸比对时误差变大
	static seeta::ModelSetting FL_model("./model/pd_2_00_pts81.dat", device, 0);
	//seeta::ModelSetting FL_model("./model/pd_2_00_pts5.dat", device, 0);
	myMutex.unlock();
	static seeta::FaceDetector FD(FD_model);
	static seeta::FaceLandmarker FL(FL_model);
	//设置人脸框的最小尺寸，不能小于20
...
// QImage转CV::Mat
cv::Mat QImage2cvMat(const QImage &inImage, bool inCloneImageData)
{
...
float getSimilar(cv::Mat& face1, std::vector<SeetaPointF>& face1Points, cv::Mat& face2, std::vector<SeetaPointF>& face2Points)
{
    ...
    static	seeta::FaceDatabase  FDB(seeta::ModelSetting("./model/fr_2_10.dat", seeta::ModelSetting::AUTO, 0));

	//清除之前导入的人脸
	FDB.Clear();
	return FDB.CompareByCroppedFace(face1Data, face2Data);
	return FDB.Compare(face1Data, face1Points.data(), face2Data, face2Points.data());
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////
seeTaFaceWindow::seeTaFaceWindow(QWidget *parent)
	: QMainWindow(parent)
{
	ui.setupUi(this);
	connect(ui.importFace1Button, &QPushButton::clicked, this, &seeTaFaceWindow::importFace1);
	connect(ui.importFace2Button, &QPushButton::clicked, this, &seeTaFaceWindow::importFace2);
	connect(ui.faceDetectionButton, &QPushButton::clicked, this, &seeTaFaceWindow::startSeetaFace);
	connect(ui.faceAlignmentButton, &QPushButton::clicked, this, &seeTaFaceWindow::startFaceAlignment);
	face1 = nullptr;
	face2 = nullptr;
	faceFlag = true;
	...
	//初始化人脸识别模型
	FDB = new seeta::FaceDatabase(seeta::ModelSetting("./model/fr_2_10.dat", seeta::ModelSetting::AUTO, 0));
}

void seeTaFaceWindow::importFace(int flag)
{
	//不能支持PNG图片
...
void seeTaFaceWindow::importFace1()
{
	importFace(1);
}

void seeTaFaceWindow::importFace2()
{
	importFace(2);
}

void seeTaFaceWindow::startSeetaFace()
{
...
void seeTaFaceWindow::startFaceAlignment()
{
	//初始化
	...
	std::thread t1(&seeTaFaceWindow::faceDetectionFunc, this, std::ref(tempFace1), std::ref(this->face1Points), "face_detection_face1");
	std::thread t2(&seeTaFaceWindow::faceDetectionFunc, this, std::ref(tempFace2), std::ref(this->face2Points),"face_detection_face2");
	t1.join();
	t2.join();
...
// 	cv::imshow("face_detection_face2", tempFace2);
	...
	ui.face1Label->setPixmap(QPixmap::fromImage(cvMat2QImage(tempFace1)));
	ui.face2Label->setPixmap(QPixmap::fromImage(cvMat2QImage(tempFace2)));

}

//人脸检测、81特征点标记
void seeTaFaceWindow::faceDetectionFunc(cv::Mat &mat, std::vector<SeetaPointF>& facePoints, std::string windName)
{
	//使用多线程处理图片，加快速度
	std::cout << std::this_thread::get_id() << endl;
...
float seeTaFaceWindow::getSimilar(cv::Mat& face1, std::vector<SeetaPointF>& face1Points, cv::Mat& face2, std::vector<SeetaPointF>& face2Points)
{
	seeta::cv::ImageData face1Data = face1;
	seeta::cv::ImageData face2Data = face2;
	//清除之前导入的人脸
...
seeTaFaceWindow::~seeTaFaceWindow()
{
